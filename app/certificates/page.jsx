  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("QR Code Data:", data.data); // Log QR Code data for debugging
        setCertificateData(data.data); // Set the certificate data to state
      } else {
  ...
  return (
    <main className="flex items-center justify-center w-full min-h-screen bg-gray-100">
      <div className="max-w-6xl">
        ...
        >
          <h1 className="mb-2 text-3xl font-medium text-center">
            <span className="text-yellow-400">Verify</span> Your Certificate
          </h1>
          <p className="mb-2 text-sm text-justify text-gray-700">
          ...
          <div className="relative mb-4">
            <p className="mb-1 text-sm font-medium">Upload your certificate:</p>
            <label
              htmlFor="certificate"
              className="block px-4 py-2 text-center text-white bg-yellow-400 rounded-full cursor-pointer hover:bg-yellow-500"
            >
              {loading ? "Verifying..." : "Select Certificate"}
            </label>
            <input
              type="file"
              name="certificate"
